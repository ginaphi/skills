...
| **`bbq`** | Crystallises a planning/grilling/design session into a plain-Markdown record in `docs/session/` — decisions made, paths rejected (and why), and open threads. Triggers on *"/bbq", "crystallise this", "write up what we decided"*. Stack-agnostic; no special tooling — it's a normal `.md`. |
| **`bump`** | Dependency-upgrade workflow for a JS/TS workspace (npm or pnpm). Triggers on *"/bump", "upgrade packages", "bump deps"*. Surveys declared versions, classifies safe / major / runtime-tied / CLI-managed, applies one PR per bucket, and verifies with the project's *own* lint/typecheck/build/test scripts. |
